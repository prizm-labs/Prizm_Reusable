...
namespace TouchScript.Gestures
{
    ...
    public class TransformGesture : TransformGestureBase, ITransformGesture
    {
        ...
        public void ApplyTransform(Transform target)
        {
			...
			if (target.localScale.sqrMagnitude > maxScaling.sqrMagnitude) {
				Debug.Log ("we were too big");
				target.localScale = maxScaling;
			}
			if (target.localScale.sqrMagnitude < minScaling.sqrMagnitude) {
				Debug.Log ("we were too  small");
				target.localScale = minScaling;
			}
